		...
		for(int a = bound[pos][0]; a <= bound[pos][1]; a += step) {
			if(a < 0) 
				...
				if(pos < this.len - 2)
					...
					if(deltaAB(LAB_ref, LAB_computed) < 6.0 && minDiff > Math.abs(LAB_ref[0] - LAB_computed[0])) {
						if(this.useCond2 == true)
							this.useCond2 = false;
						minDiff = Math.abs(LAB_ref[0] - LAB_computed[0]);
						this.computedColor = new Colors(computedData);
						finalRatio = ratioMat.toArray1D();
					}
					else if(this.useCond2 == true && minDiff2 > deltaE(LAB_ref, LAB_computed)) {
